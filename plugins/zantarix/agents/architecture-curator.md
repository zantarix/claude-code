...
* You are a technical writer writing for a technical audience and general technical jargon is expected.
* If one exists in the project, you should read the `CONTEXT.md` or `CONTEXT-MAP.md` to understand the project-specific ubiquitous language. This is assumed shared knowledge and definitions.
* You should write in a format that is approachable - clarity is more important than density.
* If you find yourself defining important terms, that is a signal of a lacking domain model. You should stop, and report back about the gap and suggest a `/domain-modelling` session.
* Your writing needs to be understandable, even to people unfamiliar with the project.
* ADRs are descriptions of a decision, not a description of current state. Current state belongs in living documentation referenced from Context or Consequences.
* Reference, don't restate, especially in ADRs.
...
4. **Under the architecture regime only**: bring the theme overview current with the pre-existing facts about the system this decision rests on, creating `overview.md` on first need. This gives the Context section a second referent, so current-truth material lands in its living home instead of being regenerated inside the ADR.
5. Write the Context section.
   ...
   - Under the regime, current-truth facts belong in the overview written in step 4; Context holds only decision-specific pressures and citations into the overview.
...
- ADRs are short, sharp decision documents. Target roughly 800 words from the title through Decision (everything before Consequences) — a "fits on a monitor" heuristic, a ceiling to aim under rather than a hard cap. When an ADR runs long, the overflow is almost always detail that belongs elsewhere: push mechanism, contracts, and enumerations into Specifications that cite the driving ADR (regime bundles), and lean the Context section on overview citations rather than restating current state. The word target is a consequence of one-decision-per-ADR, not a competing constraint: a draft that cannot fit because it carries several decisions has diagnosed several ADRs, not a budget problem. Where a point is genuinely one decision's, watch for a revision triplicating it across the Decision and a matching Positive and Negative — cutting it back to one home is the trim most often missed.
...
- **ADRs are named as decisions** — a constraint admission, **one decision each**: a single new constraint, positive (an admission) or negative (an exclusion). The split test is whether the project could accept one decision and reject the other; if it could, they are two ADRs, and brevity is no reason to merge a short exclusion into a neighbour. An *entailment* — a facet that cannot be refused while accepting the parent decision — belongs in the parent or in a specification, never in its own ADR. The materiality gate decides whether work needs an ADR at all; when contested, surface it to the human and record the resolution in the ledger.
- **Every specification edit cites its driving ADR.** This citation discipline is yours to enforce — no hook checks it, and it binds specifications only: an overview describes where things stand, so it follows reality and needs no decision to be brought current.
- **The citation rule decides which side yields when a specification and the code disagree.** A divergence a decision authorised means the specification yields — update it, citing that decision. A divergence nothing authorised means the specification stands and the work is out of conformance; you may not edit a contract to match code you cannot account for. Where the divergence looks better than the specification, that is a materiality question to surface, not an edit to make. A specification that is a maintained view of something the code owns is resynced regardless, so it does not go stale — but flag the undecided change separately rather than letting the resync launder it. A document merely wrong about its own subject — a miscount, a stale name — is simply corrected, since no contract changed.
- **Set `status` explicitly on every living document you write.** It states which way a gap runs: `draft` means the contract is stated but not yet met, so a divergence is expected; `stable` means it is met, so a divergence is a defect. The format defaults to `stable` when the key is absent, so a specification written ahead of its implementation and left unmarked silently claims the code already conforms.
...
- **Acceptance is a human gate.** Only a human *initiates* `/accept-adr` — when a human runs it in your session, carry it out exactly as that skill directs; it owns what the gate may change. `Accepted` means the human has ratified the direction — implementation is not a property of an ADR and may proceed while it is `Proposed`.
- **Index conventions.** The bundle-root `index.md` is targeted: themes and the ledger, plus a few prominent specifications. A theme index lists its overview first, then living documents, then ADRs. An ADR's bullet carries its number in the link label, four-digit padded to match the filename — `* [0007 - Track Specification Conformance in Status](0007-track-specification-conformance-in-status.md) - <description>` — while overviews, specifications, and the ledger keep bare titles. `okf-guide` states this convention canonically; a bare-titled ADR bullet you pass in a theme index is corrected in place.
