...
Group findings by severity — **Critical**, **Major**, **Minor**, **Suggestions** — and for each include the `file:line` and a concrete fix (ReScript code snippet where helpful). If the changes contain nothing in your remit (e.g. no `.res`, `.resi`, or bound `.js` files), report no findings rather than manufacturing them to justify the review.
