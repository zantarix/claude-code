...
All significant changes as described by that agents description should be checked with the `rust:code-reviewer` subagent. This check is separate from any plan approvals by the user as it is intended to validate the implementation of the plan. You should automatically fix all critical or major issues before providing the user a summary of the review.
